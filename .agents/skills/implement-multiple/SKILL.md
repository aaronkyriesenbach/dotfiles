...
Invoke with `/skill:implement-multiple`, optionally followed by the scope: an epic, a ticket count, a wave count, or an explicit ticket set/label. Examples:

- `/skill:implement-multiple` — no scope; you'll be asked to pick an epic (see step 2).
- `/skill:implement-multiple the checkout-redesign epic`
- `/skill:implement-multiple 5 tickets`
- `/skill:implement-multiple 2 waves`
- `/skill:implement-multiple all ready-for-agent`
...
Parse what the user asked for when invoking this skill — the scope passed to `/skill:implement-multiple`, or otherwise stated in conversation. If blank, you'll be asked to pick an epic — see the last bullet below.
